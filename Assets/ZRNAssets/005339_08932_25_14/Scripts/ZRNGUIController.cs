...
public class ZRNGUIController : MonoBehaviour {
	...
	void Start () {
		...
		this.GetComponent<CameraControllerCity>().ChangeCamera(0);
		...
		if (menuVisible == true)
		{
			...
			if (operateCameraNumber < 100)
			{
				...
				if (GUI.Button (new Rect (400, 60, 50, 40), "<---"))
				{
					...
					{
						operateCameraNumber = this.GetComponent<CameraControllerCity>().targetCameraNames.Count -1;
				...
				{
					operateCameraNumber++;
					if (operateCameraNumber > this.GetComponent<CameraControllerCity>().targetCameraNames.Count -1)
					{
						operateCameraNumber = 0;
						previousCameraNumber = operateCameraNumber;
					}
				}
				GUI.Label (new Rect (460, 60, 140, 20), this.GetComponent<CameraControllerCity>().targetCameraNames[operateCameraNumber]);
				if (GUI.Button (new Rect (450, 80, 150, 20), "Change"))
				{
					this.GetComponent<CameraControllerCity>().ChangeCamera(operateCameraNumber);
					previousCameraNumber = operateCameraNumber;
					SetQueryChan(0);
				}
			}
